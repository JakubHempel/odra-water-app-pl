...
def get_disaster_images():
    ...
    wroclaw_pre = clip_sentinel_disaster(pre_disaster, gd.wroclaw, "2022-07-20")
    wroclaw_dur = clip_sentinel_disaster(dur_disaster, gd.wroclaw, "2022-07-31")
    wroclaw_post = clip_sentinel_disaster(post_disaster, gd.wroclaw, "2022-08-25")

    # wroclaw_collection = ee.ImageCollection.fromImages([wroclaw_pre, wroclaw_dur, wroclaw_post]).map(water_indexes)
    ...
    # szczecin_collection = ee.ImageCollection.fromImages([szczecin_pre, szczecin_dur, szczecin_post]).map(water_indexes)
    ...
    # frankfurt_collection = ee.ImageCollection.fromImages([frankfurt_pre, frankfurt_dur, frankfurt_post]).map(water_indexes)

    ostrava_pre = clip_sentinel_disaster(pre_disaster, gd.ostrava, "2022-07-20")
    ostrava_dur = clip_sentinel_disaster(dur_disaster, gd.ostrava, "2022-07-31")
    ostrava_post = clip_sentinel_disaster(post_disaster, gd.ostrava, "2022-08-25")

    # ostrava_collection = ee.ImageCollection.fromImages([ostrava_pre, ostrava_dur, ostrava_post]).map(water_indexes)

    before_collection = ee.ImageCollection.fromImages(
        [wroclaw_pre, szczecin_pre, frankfurt_pre, ostrava_pre]
    ).map(water_indexes)
    during_collection = ee.ImageCollection.fromImages(
        [wroclaw_dur, szczecin_dur, frankfurt_dur, ostrava_dur]
    ).map(water_indexes)
    after_collection = ee.ImageCollection.fromImages(
        [wroclaw_post, szczecin_post, frankfurt_post, ostrava_post]
    ).map(water_indexes)

    return {
        "Before disaster": before_collection,
        "During disaster": during_collection,
        "After disaster": after_collection,
    }
...
def get_all_layers():
    ...
    # List of index names
    index_names = ["NDWI", "NDVI", "NDSI", "SABI", "CGI", "CDOM", "DOC", "Cya", "Turb"]
